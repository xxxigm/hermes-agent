...
class AIAgent:
    ...
    @staticmethod
    def _is_xai_oauth_bad_credentials_failure(
        error_context: Optional[Dict[str, Any]],
    ) -> bool:
        """Detect xAI OAuth 403s that are stale credentials, not entitlement.

        xAI can return the same permission-denied ``code`` for both an
        unsubscribed account and an invalid/stale OAuth token.  The explicit
        credential-validation signals below must take precedence over the
        xAI 403 refresh-loop guard so long-running sessions can refresh.
        """
    ...
    def _is_entitlement_failure(
        ...
        if AIAgent._is_xai_oauth_bad_credentials_failure(error_context):
            return False
