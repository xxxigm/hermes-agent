...
def recover_with_credential_pool(
    ...
    *,
    ...
    if effective_reason == FailoverReason.auth:
        ...
        # the refresh loop can't spin in those cases either — except when
        # xAI explicitly says the OAuth token itself failed validation
        # (#29344), because that is exactly the refreshable stale-token case.
        is_entitlement = agent._is_entitlement_failure(error_context, status_code)
        if (
            not is_entitlement
            and status_code == 403
            and (agent.provider or "") == "xai-oauth"
            and not agent._is_xai_oauth_bad_credentials_failure(error_context)
        ):
            is_entitlement = True
        if is_entitlement:
            _ra().logger.info(
