...
KANBAN_GUIDANCE = (
    ...
    "eyes on it.\n"
    "5a. **Verify artifacts before claiming done.** When your task's "
    "definition of done is 'X exists' (a cron job, a created Kanban "
    "card, a deployed service, a sent email…), do not just write `summary"
    "='Created X'` and call `kanban_complete`.  Pass the structured "
    "manifest fields so the kernel can verify your claim:\n"
    "    - `created_cards=[...]` for new Kanban task ids you got back "
    "from `kanban_create`.\n"
    "    - `created_artifacts=[{kind, id}, ...]` for non-Kanban side-"
    "effects.  Today only `kind='cron'` is verified end-to-end (the "
    "kernel calls `cron.jobs.get_job(id)` and rejects the completion "
    "with a structured error if the id does not exist) — but list any "
    "kind that ships an id and the audit log will record your claim "
    "even when the verifier hasn't been written yet.\n"
    "    If a verifier rejects your manifest you get a tool_error "
    "naming the phantom ids; the task stays in-flight (no state "
    "change), so the right response is to either (a) actually create "
    "the missing artifact and retry with the real id, (b) drop the "
    "bogus entry and retry, or (c) `kanban_block` with the underlying "
    "reason.  Do NOT mark the task done while the artifact is missing "
    "— that's the failure mode reported in issue #25288 and the "
    "kernel will block it anyway.\n"
    "5b. **Use idempotency_key when you may retry a creation.** When "
    "you create something whose name doesn't carry a stable identity "
    "(cron jobs are the canonical example — names are auto-derived "
    "from the prompt and ids are random), pass an `idempotency_key` "
    "you can recompute deterministically (a hash of the prompt + "
    "schedule, your `$HERMES_KANBAN_TASK` id, etc.).  A retried "
    "creation with the same key returns the prior job with "
    "`reused=true` instead of producing a duplicate.\n"
    ...
    "- Do not claim an artifact (cron job, created card, deployed service) "
    "exists without an id you can put in `created_artifacts` / "
    "`created_cards`. The kernel verifies these and will reject the "
    "completion if you invent ids — see issue #25288.\n"
