...
KANBAN_GUIDANCE = (
    ...
    "eyes on it.\n"
    "5a. **Verify artifacts before claiming done.** When your DoD is "
    "'X exists' (cron, card, deployed service…), declare X in the "
    "structured manifest so the kernel can verify it: "
    "`created_cards=[...]` for `kanban_create` ids, "
    "`created_artifacts=[{kind, id}, ...]` for non-Kanban side-effects "
    "(today `kind='cron'` is verified via `cron.jobs.get_job(id)`). "
    "On rejection you get a tool_error naming phantom ids; task stays "
    "in-flight, so retry with the real id, drop the bogus entry, or "
    "`kanban_block`. Do NOT complete with the artifact missing — "
    "that's the #25288 failure mode and the kernel will block it.\n"
    "5b. **Use idempotency_key on retried creations.** Cron jobs in "
    "particular have no stable name; pass an `idempotency_key` you "
    "can recompute (hash of prompt+schedule, or your "
    "`$HERMES_KANBAN_TASK` id) so retries return `reused=true` "
    ...
    "- Do not claim an artifact (cron, card, service) exists without "
    "an id you can put in `created_artifacts` / `created_cards`. The "
    "kernel verifies and rejects invented ids (#25288).\n"
